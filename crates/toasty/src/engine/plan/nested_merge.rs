...
impl NestedMergePlanner<'_> {
        ...
        let ret = match stmt_state.stmt.as_deref().unwrap() {
            ...
            stmt::Statement::Update(update) => NestedChild {
                ...
                single: update
                    .returning
                    .as_ref()
                    .is_none_or(stmt::Returning::is_single),
            },
    ...
    fn plan_nested_level(&mut self, stmt_id: hir::StmtId, depth: usize) -> NestedLevel {
            ...
            && let stmt::ExprSet::Values(values) = &query.body
        {
            ...
            match &stmt.returning_unwrap().expr {
                stmt::ReturningExpr::Project(expr) => {
                    let (s, _) = self
                        .inputs
                        .insert_full(stmt_state.load_data_statement.get().unwrap());
    ...
    fn build_projection_from_expr(
        ...
        visit_mut::walk_expr_scoped_mut(&mut projection, 0, |expr, scope_depth| match expr {
            stmt::Expr::Arg(expr_arg) if expr_arg.nesting == scope_depth => {
                ...
                match &stmt_state.args[position] {
                    hir::Arg::Sub {
                        ...
                        match child_stmt.returning().map(|returning| &returning.expr) {
                            Some(stmt::ReturningExpr::Expr(returning_expr))
            ...
            stmt::Expr::Project(project)
                if matches!(
                    project.base.as_ref(),
                    stmt::Expr::Row(stmt::ExprRow::Old(stmt::ExprRowTarget::Table(_)))
                ) =>
            {
                ...
                let index = selection.get_index_of_old_column(*table, *column);
    ...
    fn build_filter_for_nested_child(
        ...
        visit_mut::for_each_expr_mut(&mut filter, |expr| match expr {
            stmt::Expr::Arg(expr_arg) => {
                ...
                let target_exec_statement_index = if target_stmt
                    .stmt()
                    .returning()
                    .is_some_and(stmt::Returning::uses_old)
                    && let stmt::Statement::Update(update) = target_stmt.stmt()
                    && let stmt::UpdateTarget::Table(table) = update.target
                    && let stmt::ExprReference::Column(column) = target_expr_ref
                {
                    target_selection.get_index_of_old_column(table, column.column)
                } else {
