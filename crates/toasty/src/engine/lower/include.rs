...
//! The first thing we do is flatten any `PathRoot::Variant` chain into a plain
//! projection of the form `[…parent_steps, variant_idx, …local_var_field_steps]`.
//! This LOCAL form mirrors the IR's `Match` arm record (where each arm is
//! `[disc, field_0, field_1, …]`), so descent through enum variants is just
//! two index steps: one selecting the arm by variant index, the next addressing
//! a local variant field.
//!
//! The flow:
//!
//! ```text
...
impl LowerStatement<'_, '_> {
    ...
    /// fields at positions `1..`. `sub_includes` are the path remainders that
    ...
    fn build_include_subquery(
        ...
        returning[field_index] =
            self.build_relation_subquery_inner(field_index, nested, top_filter);
    }
    ...
    pub(super) fn build_relation_subquery(
        ...
        let flat: Vec<FlatInclude> = nested
            ...
            .collect();
        self.build_relation_subquery_inner(field_index, &flat, top_filter)
    }
...
/// Flatten a [`stmt::Path`] into a single projection.
